...
function openProfile() {
    ...
    if (nameEl) nameEl.textContent = user.name || "Loop Member";
    ...
    if (trustEl) trustEl.textContent = user.trust_score || 0;
    ...
    if (listingsBox) {
        if (!myItems.length) {
            ...
            listingsBox.innerHTML = myItems.map(item => `
                <div class="profile-list-item" style="padding: 10px; border-bottom: 1px solid #eee; display: flex; justify-content: space-between;">
                    <strong>${escapeHtml(item.title)}</strong> 
                    <span>${escapeHtml(item.transaction_type)} (${item.price ? '₹' + item.price : 'Barter'})</span>
                </div>
            `).join("");
        }
    }

    // Safely open the profile modal
